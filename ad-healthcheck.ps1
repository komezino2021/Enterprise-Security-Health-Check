...
ForEach($Domain in $Domains | sort { $_.length }){
    ...
    Write-Output ""
    Get-ADUser -Filter * -Properties LastLogonDate -server $Domain | Where-Object {($_.Enabled -eq $true) -and ($_.LastLogonDate -lt (Get-Date).AddDays(-70)) -and ($_.LastLogonDate -ne $null)} | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize

    Write-Output ""
    Write-Output "Enabled Active Directory user accounts older than 30 days that have never authenticated"
    Write-Output ""
    Get-ADUser -Filter * -Properties LastLogonDate -server $Domain | Where-Object {($_.Enabled -eq $true) -and ($_.LastLogonDate -eq $null) -and ($_.whenCreated -lt (Get-Date).AddDays(-30))} | select name, distinguishedName, whenCreated |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADUser -Filter * -Properties LastLogonDate -server $Domain | Where-Object {($_.LastLogonDate -lt (Get-Date).AddDays(-100)) } | select name, distinguishedName, LastLogonDate|Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADUser -Filter * -Properties * -Server $Domain | Where { $_.UserAccountControl -band 0x0080 } | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize

    Write-Output ""
    Write-Output "User accounts that can use DES to authenticate (weak encryption)"
    Write-Output ""
    Get-ADUser -Filter {UserAccountControl -band 0x200000} -server $Domain | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADUser -Filter {DoesNotRequirePreAuth -eq $true} -server $Domain | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADComputer -Filter { (TrustedForDelegation -eq $True) -AND (PrimaryGroupID -ne '516') -AND (PrimaryGroupID -ne '521') } -server $Domain | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADUser -Filter * -Properties * -server $Domain  | Where { ($_.LastLogonDate -le $LastLoggedOnDate) -AND ($_.PasswordLastSet -le (Get-Date).AddDays(-60)) } | select name, distinguishedName, LastLogonDate, PasswordLastSet |Format-Table -AutoSize

    Write-Output ""
    Write-Output "User accounts that with passwords that never expire"
    Write-Output ""
    get-aduser -filter * -server $Domain -properties Name, PasswordNeverExpires | where {$_.passwordNeverExpires -eq "true" } |  select name, distinguishedName, LastLogonDate|Format-Table -AutoSize

    Write-Output ""
    Write-Output "User accounts that do not require a password (even though they may have one)"
    Write-Output ""
    get-ADUser -server $Domain -Filter {PasswordNotRequired -eq $true} | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADComputer -server $Domain -Filter * -Properties LastLogonDate | Where-Object {($_.enabled -eq $true ) -and ($_.lastLogonDate -ne $null) -and ($_.LastLogonDate -lt (Get-Date).AddDays(-70)) } | select name, distinguishedName, LastLogonDate |Format-Table -AutoSize
    
    Write-Output ""
    Write-Output "Enabled computer accounts older than 30 days which have never authenticated "
    Write-Output ""
    Get-ADComputer -Filter * -Properties LastLogonDate -server $Domain | Where-Object {($_.Enabled -eq $true) -and ($_.LastLogonDate -eq $null) -and ($_.whenCreated -lt (Get-Date).AddDays(-30))} | select name, distinguishedName |Format-Table -AutoSize
    ...
    Write-Output ""
    Get-ADComputer -Filter * -Properties LastLogonDate -server $Domain | Where-Object {($_.LastLogonDate -lt (Get-Date).AddDays(-100)) } | select name, distinguishedName, LastLogonDate|Format-Table -AutoSize
